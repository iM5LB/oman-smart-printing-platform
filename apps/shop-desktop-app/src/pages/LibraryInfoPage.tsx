  ...
  return (
    <div className="flex items-center justify-between gap-3 border-b border-border-default/50 px-3 py-2 last:border-b-0">
      <div className="flex min-w-0 items-center gap-2">
        {icon ? <span className="shrink-0 text-primary/70">{icon}</span> : null}
...
function Chip({
  ...
  return (
    ...
    >
      ...
      <span className="truncate text-meta font-semibold text-text-primary">{value}</span>
    </div>
  );
}
          ...
          <div className="shrink-0">
            ...
            <Field label="رابط العملاء" icon={Icons.bag({ size: 13 })} dir="ltr">
              {shopUrl ? (
                <button
                  type="button"
                  onClick={() => void openExternalUrl(shopUrl)}
